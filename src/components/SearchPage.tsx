        ...
        {filtered.length === 0 ? (
          <div className="space-y-6">
            <div className="rounded-2xl bg-white border border-ink-200 p-8 sm:p-12 text-center">
              ...
              <p className="text-sm text-primary-500 mt-1 max-w-md mx-auto">We couldn't find services matching your filters. Try a different zip code, broaden your keywords, or clear filters to see what's available nearby.</p>
              <div className="mt-5 flex flex-wrap items-center justify-center gap-2">
                <button onClick={handleClear} className="px-4 py-2 rounded-lg bg-primary-700 text-white text-sm font-medium hover:bg-primary-800 transition-all duration-200 ease-out-expo">Clear all filters</button>
              ...
              <div>
                ...
                <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
                  {featuredServices(resources, 6).map((resource, idx) => (
                    <ResourceCard key={resource.id} resource={resource} category={categories.find((c) => c.id === resource.category_id)} onClick={() => onNavigate({ name: 'resource', id: resource.slug || resource.id })} animationDelay={Math.min(idx * 30, 300)} searchQuery={deferredSearch} />
                  ))}
                </div>
              </div>
            )}
...
function ResourceCard({ resource, category, onClick, animationDelay, searchQuery }: { resource: ResourceWithCategory; category?: ResourceCategory; onClick: () => void; animationDelay: number; searchQuery: string }) {
      ...
      <div className="p-3.5 flex-1 flex flex-col">
        <div className="flex items-start justify-between gap-2">
          <h3 className="font-display font-bold text-sm text-primary-800 leading-tight group-hover:text-primary-700 transition-colors line-clamp-2">
            {highlightSegments(resource.name, searchQuery).map((seg, i) => seg.match ? <mark key={i} className="bg-sage-200 text-primary-900 rounded px-0.5">{seg.text}</mark> : seg.text)}
          </h3>
          {hasRating(resource.rating) ? <div className="flex items-center gap-0.5 flex-shrink-0"><Star className="w-3 h-3 text-warning-500 fill-warning-500" /><span className="text-xs font-bold text-primary-700">{resource.rating.toFixed(1)}</span></div> : null}
        </div>
        <p className="mt-1 text-xs text-primary-500 line-clamp-2 leading-relaxed">
          {highlightSegments(resource.description, searchQuery).map((seg, i) => seg.match ? <mark key={i} className="bg-sage-200 text-primary-900 rounded px-0.5">{seg.text}</mark> : seg.text)}
        </p>
